...
1. **用户中心**：用于集中管理多份本地简历。这里通常会展示简历列表、搜索和排序入口，以及批量删除、导入、导出等管理操作，适合作为日常维护简历库的入口。

   ![image-20260412220825130](./docs/user-center.png)

2. **编辑和预览同屏**：适合边编辑边确认最终排版。用户可以一边调整模块内容、顺序和样式，一边查看预览区域的实时渲染效果，减少反复切换页面的成本。

   ![image-20260412220926181](./docs/edit-preview.png)

3. **仅编辑模式**：适合专注录入和整理内容。当用户希望连续补全工作经历、项目说明或教育信息时，可以聚焦在编辑区域，减少预览干扰。

   ![image-20260412221004151](./docs/edit-only.png)

4. **仅预览模式**：适合最终检查展示效果。用户可以专门查看页面层级、排版密度、模块顺序和视觉节奏，确认导出前的成品观感。
   ![image-20260412221038373](./docs/preview-only.png)

5. **自由布局能力**：支持不同模块组合、重排以及多行布局。对于项目经历、技能标签、附加信息等内容，可以按照实际投递需要灵活组织结构。

   ![image-20260412221116169](./docs/.png)

6. **多种导出方式**：同一份简历可以导出为 PDF、PNG、JPG、WEBP、SVG、JSON 等不同格式。PDF 更适合正式投递，图片格式适合快速分享或嵌入作品集，JSON 适合备份、迁移和模板复用。

   ![image-20260412221257829](./docs/export.png)

7. **标签功能**：适合快速表达技能关键词、项目亮点、岗位匹配点或补充说明。相比长段落文字，标签更便于突出重点，也方便调整信息密度。
